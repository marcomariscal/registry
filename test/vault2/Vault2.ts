...
import type { Vault2 as Vault } from "../../src/types/Vault2";
...
describe("Vault2", async () => {
  ...
  before(async () => {
    ...
    const vaultArtifact = await artifacts.readArtifact("Vault2");
    vault = <Vault>await deployContract(admin, vaultArtifact, [token.address, "Test Vault", "aTST"]);
  });
  ...
  describe("with no deposits", async () => {
    ...
    it("should deposit if enough funds", async () => {
      ...
      // before deposit vault token supply and balance checks
      expect(await vault.totalSupply()).to.equal(ZERO);
      expect(await vault.balanceOf(admin.address)).to.equal(ZERO);
      ...
      // after deposit vault token supply and balance checks
      expect(await vault.balanceOf(admin.address)).to.equal(adminBalance);
    });
    ...
    describe("with deposits", async () => {
      ...
      it("should allow withdrawal", async () => {
        ...
        // before withdrawal vault token supply and balance checks
        expect(await vault.balanceOf(admin.address)).to.equal(adminBalance);
        ...
        // after withdrawal vault token supply and balance checks
        expect(await vault.totalSupply()).to.equal(ZERO);
        expect(await vault.balanceOf(admin.address)).to.equal(ZERO);
      });
    });
